...
@section('styles')
    <style>
        ...
        .main-wrapper::before {
            ...
            /* Ajuste l'opacité (0.65) selon ton image */
            z-index: 0;
        }
        ...
        .hero-featured-title {
            ...
            background: linear-gradient(45deg, var(--primary-text-color, #ffffff), var(--primary-color));
        ...
        .product-card {
            ...
            /* Crée le flou sur l'image de fond */
...
$heroBgUrl = str_starts_with($heroBgImage, 'images/') ? asset($heroBgImage) : asset('storage/' . $heroBgImage);

// Récupération du produit en vedette pour la section hero
$featuredProduct = \App\Models\Product::where('is_featured', true)->first();
    @endphp
        ...
        <div class="content-container">
            ...
            <section class="hero-section">
                <div class="hero-left">
                    @if ($featuredProduct)
                        @php
                            $heroImage = $featuredProduct->homepageImage();
                        @endphp
                        @if ($heroImage)
                            <img src="{{ asset('storage/' . $heroImage->image_path) }}" alt="{{ $featuredProduct->name }}"
                                class="hero-featured-img">
                        @else
                            <img src="{{ asset('images/No-Product-Image-Available.webp') }}"
                                alt="{{ $featuredProduct->name }}" class="hero-featured-img">
                        @endif
                        <div class="hero-featured-title">{{ $featuredProduct->name }}</div>
                    @else
                        <img src="{{ asset('storage/products/default_1.webp') }}" alt="Berserk Sword Tee"
                            class="hero-featured-img">
                ...
                <div class="hero-right">
                    <h1 class="hero-title">{{ $featuredProduct->name ?? 'THE ECLIPSE<br>COLLECTION' }}</h1>
                    <p class="hero-subtitle">{{ $featuredProduct->short_description ?? 'Limited Edition Drop' }}</p>
                    <a href="{{ $featuredProduct ? route('products.show', $featuredProduct->id) : '#collection' }}"
                        class="btn-shop">SHOP THE LOOK</a>
                </div>
            </section>
