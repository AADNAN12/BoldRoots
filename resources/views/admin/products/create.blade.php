                    ...
                    <form id="productForm" action="{{ route('admin.products.store') }}" method="POST"
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="validation-error" id="error_description">La description doit contenir au
                                            moins 50 caractères</div>
                                        <small class="text-muted">Minimum 50 caractères - <span id="char_count">0</span>
                                            caractères</small>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label">PDF des détails du produit</label>
                                        <input type="file" class="form-control" name="product_details_pdf"
                                            accept="application/pdf">
                                        <small class="text-muted">Format: PDF (Max: 10MB). Ce PDF sera affiché dans la page
                                            produit.</small>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <div class="validation-error" id="error_category">Veuillez sélectionner une
                                            catégorie</div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control" name="compare_price"
                                                id="product_compare_price" step="0.01" min="0">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" name="new_product_badge"
                                                id="product_new_product_badge"
                                                placeholder="Ex: Nouveau, Tendance, Exclusif" maxlength="50">
                                        </div>
                                        <small class="form-text text-muted">Texte personnalisé pour le badge du produit
                                            (max 50 caractères)</small>
                                    </div>
                                    <div class="mb-3">

                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="is_new"
                                                id="product_is_new" value="1" checked>
                                        ...
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="is_featured"
                                                id="product_is_featured" value="1">
                                            <label class="form-check-label" for="product_is_featured">Produit en
                                                vedette</label>
                                        </div>
                                        <div class="form-check">
                        ...
                        <div class="form-step" data-step="2">
                            ...
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    ...
                                    <div class="validation-error" id="error_colors">Veuillez sélectionner au moins une
                                        couleur</div>
                                </div>
                                        ...
                                        @foreach ($sizes as $size)
                                            <div class="size-checkbox">
                                                <input type="checkbox" id="size_{{ $size->id }}"
                                                    value="{{ $size->id }}" data-size-name="{{ $size->value }}">
                                                <label for="size_{{ $size->id }}">{{ $size->value }}</label>
                                            </div>
                                        @endforeach
                                    </div>
                                    <div class="validation-error" id="error_sizes">Veuillez sélectionner au moins une
                                        taille</div>
                                </div>
                            </div>
                            ...
                            <div id="variants_container">
                                <div class="alert alert-info">
                                    <i class="mdi mdi-information"></i> Sélectionnez les couleurs et tailles, puis cliquez
                                    sur "Générer les variantes"
                                </div>
                            </div>
                        ...
                        <div class="form-step" data-step="3">
                            ...
                            <div id="images_container">
                                <div class="alert alert-info">
                                    <i class="mdi mdi-information"></i> Les sections d'upload apparaîtront après avoir
                                    généré les variantes
                                </div>
                            </div>
...
@section('scripts')
    ...
    <script>
        ...
        function generateVariants() {
            ...
            variantsData = [];
            let html =
                '<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Couleur</th><th>Taille</th><th>Quantité *</th><th>Seuil alerte</th><th>SKU</th></tr></thead><tbody>';
            ...
            $('.variant-quantity').on('input', function() {
                ...
                const threshold = parseInt($(this).closest('tr').find('input[name*="low_stock_threshold"]')
                    .val()) || 5;
        ...
        function generateImageSections() {
            ...
            selectedColors.forEach(colorId => {
                const colorName = $(`#color_${colorId}`).data('color-name');
                const colorCode = $(`#color_${colorId}`).siblings('label').find('.color-swatch').css(
                    'background-color');
    ...
    `;
            ...
            Object.keys(imagesData).forEach(colorId => {
                imagesData[colorId].forEach(img => {
                    hasImages = true;
                    gallery.append(
                        `<div class="image-preview-item"><img src="${img.preview}" alt="Image"></div>`);
                });
            });
        ...
        function saveToLocalStorage() {
            const formData = {
                ...
                new_product_badge: $('#product_new_product_badge').val(),
        ...
        function loadFromLocalStorage() {
            ...
            if (saved) {
                ...
                $('#product_new_product_badge').val(formData.new_product_badge);
            }
        }
        ...
        function showAutoSaveIndicator(message = 'Sauvegarde automatique effectuée') {
            ...
            if (indicator.length === 0) {
                ...
                $('body').append(
                    '<div id="autoSaveIndicator" style="position: fixed; top: 20px; right: 20px; background: #0acf97; color: white; padding: 10px 20px; border-radius: 5px; z-index: 9999; display: none;"></div>'
                );
            }
