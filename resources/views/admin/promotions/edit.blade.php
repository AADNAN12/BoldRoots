    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">Modifier la promotion</h4>
            </div>
        </div>
    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" name="type" id="promotion_type"
                                    required>
                                    <option value="">Sélectionner...</option>
                                    <option value="flash_deal" {{ old('type', $promotion->type) == 'flash_deal' ? 'selected' : '' }}>Flash Deal</option>
                                    <option value="regular_sale" {{ old('type', $promotion->type) == 'regular_sale' ? 'selected' : '' }}>Promotion régulière</option>
                                    <option value="buy_x_get_y" {{ old('type', $promotion->type) == 'buy_x_get_y' ? 'selected' : '' }}>Achetez X, obtenez Y</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('scope') is-invalid @enderror" name="scope" id="scope"
                                    required>
                                    <option value="">Sélectionner...</option>
                                    <option value="product" {{ old('scope', $promotion->scope) == 'product' ? 'selected' : '' }}>Produits spécifiques</option>
                                    <option value="collection" {{ old('scope', $promotion->scope) == 'collection' ? 'selected' : '' }}>Catégories</option>
                                    <option value="cart" {{ old('scope', $promotion->scope) == 'cart' ? 'selected' : '' }}>Panier complet</option>
                                </select>
                                @error('scope')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('discount_type') is-invalid @enderror" name="discount_type"
                                    ...
                                    <option value="fixed_amount" {{ old('discount_type', $promotion->discount_type) == 'fixed_amount' ? 'selected' : '' }}>Montant fixe (MAD)</option>
                                </select>
                                @error('discount_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control @error('discount_value') is-invalid @enderror"
                                        name="discount_value" value="{{ old('discount_value', $promotion->discount_value) }}" step="0.01" min="0"
                                        required>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" name="buy_quantity"
                                        value="{{ old('buy_quantity', $promotion->buy_quantity ?? 1) }}" min="1">
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Quantité gratuite *</label>
                                    <input type="number" class="form-control" name="get_quantity"
                                        value="{{ old('get_quantity', $promotion->get_quantity ?? 1) }}" min="1">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <select class="form-control select2-multiple" name="product_ids[]" multiple="multiple"
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}"
                                        {{ in_array($product->id, old('product_ids', $promotion->products->pluck('id')->toArray())) ? 'selected' : '' }}>
                            ...
                            <select class="form-control select2-multiple" name="category_ids[]" multiple="multiple"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ in_array($category->id, old('category_ids', $promotion->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="col-lg-4">
                ...
                <div class="card bg-light">
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            <strong>Utilisations:</strong> {{ $promotion->usage_count ?? 0 }}
                            @if($promotion->max_uses)
                                / {{ $promotion->max_uses }}
                            @endif
                        </div>
                        <div class="mb-2">
                            <strong>Créée le:</strong> {{ $promotion->created_at->format('d/m/Y H:i') }}
                        </div>
                        <div>
                            <strong>Modifiée le:</strong> {{ $promotion->updated_at->format('d/m/Y H:i') }}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('start_date') is-invalid @enderror"
                                name="start_date" id="start_date" value="{{ old('start_date', $promotion->start_date->format('Y-m-d H:i')) }}" required>
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('end_date') is-invalid @enderror" name="end_date"
                                id="end_date" value="{{ old('end_date', $promotion->end_date->format('Y-m-d H:i')) }}" required>
                            @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" class="form-control" name="min_cart_value"
                                    value="{{ old('min_cart_value', $promotion->min_cart_value) }}" step="0.01" min="0">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="max_uses" value="{{ old('max_uses', $promotion->max_uses) }}"
                                min="1">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="max_per_customer"
                                value="{{ old('max_per_customer', $promotion->max_per_customer) }}" min="1">
                            <small class="text-muted">Laisser vide pour illimité</small>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="stop_when_stock_below"
                                value="{{ old('stop_when_stock_below', $promotion->stop_when_stock_below ?? 0) }}" min="0">
